...
I applied Moran’s I test to check for spatial autocorrelation in the residuals. Significant spatial autocorrelation would indicate that spatial patterns exist in the model's residuals, potentially suggesting that spatial dependencies were not fully accounted for in the model.
...
The function `bw.gwr()` calculates the optimal bandwidth for the GWR model based on cross-validation. Here, a **fixed bandwidth** is calculated, where the bandwidth size remains constant across all locations. The "gaussian" kernel and non-adaptive bandwidth are specified.
...
With the computed bandwidth, `gwr.basic()` fits a GWR model using the fixed bandwidth. This model will provide local estimates of the coefficients for each variable at each data point.
...
Here, `bw.gwr()` calculates an **adaptive bandwidth**, where the bandwidth size adjusts based on location density. This approach is often better for data with varying spatial distributions, as it accommodates denser or sparser regions differently.
...
This creates the adaptive bandwidth GWR model, which again provides local estimates of the predictors but with an adaptive bandwidth.
...
The Spatial Data Frame (SDF) from the adaptive GWR model output is converted to an `sf` object with the appropriate CRS for easy visualization in `tmap`.
...
This adds the adaptive GWR model results (e.g., coefficient estimates and local R-squared values) to the condo resale dataset for further analysis and visualization.
...
This map displays the local R² values from the GWR model, showing how well the model explains the variance in condo prices across different locations.
...
Two maps display the standard error (SE) and t-values (TV) of the `AREA_SQM` coefficient, allowing visualization of its stability and significance across locations.
...
This map shows the `Local_R2` values for properties in the Central Region, providing insights into spatial variation in model fit within a specific planning area.
